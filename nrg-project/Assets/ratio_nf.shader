            ...
            Pass
            {
                ...
                float4 T(float3 x, float3 y, float3 dir)
                {
                    ...
                    do {
                        ...
                        //sc = tex3D(_Volume, x + t * dir + float3(0.5f, 0.5f, 0.5f), 0, 0); // sample cloud texture
                        ...
                        float cT = exp(-_MaxDensity * (d - t));
                        Tm = Tm * (1-sc);
                        T = T + Tm * cT;
                        cp++;
                    } while (true);

                    //return float4(T, cp/500, 0, 1);
                ...
                void frag(v2f i, out float4 color : SV_Target)
                {
                    ...
                    if (unity_OrthoParams.w)
                        color = T(float3(hpos.x, hpos.y, hpos.z), float3(hpos.x, hpos.y, hpos.z + 1), dir); // rays go straight through the cube
                    else {
