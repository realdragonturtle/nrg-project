                ...
                float4 T(float3 x, float3 y, float3 dir)
                {
                    ...
                    do {
                        if (i > 10000)
                            break;
                        i++;
                        t = t - log(1 - rand(i * x + _Time.y)) / _MaxDensity;
                        if (t >= d)
                            break;
                        e = rand(y + i + _Time.y);

                        //sc = tex3D(_Volume, x + t * dir + float3(0.5f, 0.5f, 0.5f), 0, 0); // sample texture
                        float3 pos = x + t * dir;
